...
final class PreHooksStep implements StepInterface
{
    ...
    {
        $config = $context->getStepConfig('pre_hooks');
        $phase = $context->packageBuilder !== null ? 'export' : 'import';
        $scriptsKey = $phase . '_scripts';
        $scripts = $config[$scriptsKey] ?? [];

        if (empty($scripts)) {
            return StepResult::skipped('No pre-hook scripts configured for ' . $phase);
        }
        ...
        foreach ($scripts as $script) {
            ...
            if (!$result->isSuccessful()) {
                return StepResult::failed(
                    sprintf('Pre-hook script failed: %s (exit code %d)', $script, $result->exitCode),
